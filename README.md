...
| 🛡️ Host verification | `known_hosts`-backed, trust-on-first-use with an explicit fingerprint confirmation; a **changed** host key is flagged as a possible attack and requires an extra confirmation; Left/Right arrow keys move between the Yes/No buttons, same as Tab |
